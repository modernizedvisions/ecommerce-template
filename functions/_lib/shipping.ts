...
const buildCategoryShippingMap = (categories: ShippingCategory[]) => {
  const map = new Map<string, number>();
  categories.forEach((cat) => {
    const rawCents = cat.shippingCents ?? cat.shipping_cents;
    const shippingCents =
...
// Centralized shipping rule for server calculations (must match frontend helper).
export function calculateShippingCents(items: ShippingItem[], categories: ShippingCategory[]): number {
